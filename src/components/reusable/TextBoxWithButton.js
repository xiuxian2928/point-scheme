...
function TextBoxWithButton({value, color, title, onClick, onChange, hasOption=false}){
    const OPTIONS = [
        { value: "0", name: "--선택--" },
    ...
    const optionView = function(){
        if(hasOption){
            return <SelectBox options={OPTIONS} onChange={onChange}></SelectBox>
        }
    }
